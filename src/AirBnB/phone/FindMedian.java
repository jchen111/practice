...
public class FindMedian {
    ...
    private static long search(int[] nums, int k, long small, long large) {
        if (small == large) {
            return small;
        }
        ...
        if (count == k) {
            return res;
        } else if (count < k) { // guess is to small
            return search(nums, k, Math.max(res + 1, guess), large);
        } else { // guess is to big
            return search(nums, k, small, res);
        }
    }

      public static void main(String[] args) {
        System.out.println(findMedian(new int[] {-6, 18, 9}));
        System.out.println(findMedian(new int[] {-20, 0, 7, 5}));
        System.out.println(findMedian(new int[] {1, 2, 2, 2, 3, 3}));
      }
}
