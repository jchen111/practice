    ...
    public static int shortenUrlToId(String url) {
        ...
        for(int i = 0; i < url.length(); i++){
            if(url.charAt(i) >= '0' && url.charAt(i) <= '9'){
                id += Character.getNumericValue(url.charAt(i)) * Math.pow(62,url.length()-1-i);
            }else if(url.charAt(i) >= 'a' && url.charAt(i) <= 'z'){
                id += (10 + url.charAt(i) - 'a') * Math.pow(62,url.length()-1-i);
            }else{
                id += (36 + url.charAt(i) - 'A') * Math.pow(62,url.length()-1-i);
            }
        }
        return id;
    }
